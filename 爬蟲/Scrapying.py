...
def Scrapying(url):
    ...
    data = pd.DataFrame(games_detail)
    data.to_csv("./Season_data/Season_%s_data.csv" % year,mode='a', encoding='utf_8_sig',header=False)
    error_csv = pd.DataFrame(error_list)
    error_csv.to_csv("./Season_error_list/Season_%s_error_list.csv" % year,mode='a', encoding='utf_8_sig',header=False)
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor() as ex:
        ex.map(Scrapying,odds_list)

end_1 = time.time()
print('%s賽季爬蟲完成，共%s筆網頁，耗時%0.2f秒' % (year, (len(game_detail3) + len(error_list)), end_1 - start_1))
